...
import extraInd02 from '../content/extra/IND-02.json';

const manifests = { 'IND-02': ind02 };
const extras = { 'IND-02': extraInd02 };
...
export const TIPOS = [
  ...
  { id: 'quizzes', nome: 'Quizzes de autoestudo', chave: 'quiz', icone: 'quiz', fonte: 'extra', ancora: 'sec-quiz', desc: 'Perguntas com explicação a cada resposta e selo de conclusão.' },
  { id: 'mapas-mentais', nome: 'Mapas mentais', chave: 'mindmap', icone: 'mapa', fonte: 'extra', ancora: 'sec-mapa', desc: 'A estrutura de cada artigo em um mapa interativo.' },
  { id: 'historias', nome: 'Histórias visuais', chave: 'historia', icone: 'historia', fonte: 'extra', rota: 'historia', desc: 'Data storytelling: os dados do artigo contados passo a passo.' },
...
export function comAsset(chave, fonte) {
  return PUBLICACOES.filter((p) => {
    if (fonte === 'extra') return Boolean(extraDe(p.id)?.[chave]);
    const m = p.manifest;
    if (!m) return false;
...
export function destinoDe(p, tipo) {
  if (tipo.fonte === 'extra') {
    return tipo.rota
      ? `/artigos/${p.slug}/${tipo.rota}/`
      : `/artigos/${p.slug}/#${tipo.ancora}`;
  }
  return p.manifest.assets[tipo.chave] || p.manifest.links[tipo.chave];
}
